...
export default function FloatingImage({
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      const nextTarget =
        targets.find(
          t => t.getBoundingClientRect().top + window.scrollY - 10 > scrollY
      ...
      const startScroll = Math.max(
        0,
        startRect.top + window.scrollY - window.innerHeight * 0.6
      );
      const endScroll = Math.max(
        startScroll + 1,
        targetRect.top + window.scrollY - window.innerHeight * 0.25
      );
  ...
  return (
    <img
      ...
      style={{
        ...
        cursor: 'pointer',
        transition:
          'opacity 220ms var(--anim-ease), transform 220ms var(--anim-ease)',
